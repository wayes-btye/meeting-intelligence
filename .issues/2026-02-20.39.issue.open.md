...
```
Or simply restart your machine — the process won't survive a reboot.
...
**5.** Upload a `.vtt` file → confirm extraction results appear (action items, decisions, topics)
**6.** `/chat` → ask "What decisions were made about the budget?" → confirm answer + source citations rendered
**7.** `/meetings` → confirm 30+ meetings listed → click a row → confirm detail panel shows extracted items
**8.** Upload page strategy selector → confirm chunking strategy toggles between naive/speaker_turn
...
### **7. Vercel deployment** (depends on Issue #31 — Cloud Run first)

**1.** vercel.com → New Project → Import `wayes-btye/meeting-intelligence`
...
### ✅ Already done (no manual action needed)

**GitHub Secrets** — Both `CLAUDE_CODE_OAUTH_TOKEN` and `GH2MD_TOKEN` are confirmed working:
...
The git worktree references are already removed. The physical directories (`C:\meeting-intelligence-wt1-issue-22`, `wt3-issue-23`, `wt5-issue-32`) are just regular folders on disk — git no longer tracks them.

**To delete them:** Open File Explorer → navigate to `C:\` → delete the three folders manually. That is all that is needed. There is no git state inside them that needs to be preserved.

---

### 🔁 Integration tests (expensive) — PASSED

Ran `pytest -m expensive tests/test_pipeline_integration.py -v` against live API + Supabase:
- `test_full_ingest_and_query_pipeline` ✅ PASSED (44s)
- `test_ingest_stores_chunks_in_supabase` ✅ PASSED
- `test_query_without_relevant_meeting_returns_graceful_response` ✅ PASSED

All 3/3 passed. Test data was cleaned up from Supabase automatically.

---

### ⚠️ Known issue discovered during UI testing

**Extract endpoint returns 529 → 500 with no CORS headers**
...
This is a transient API condition — not a code architecture bug. The extraction itself works correctly when the Claude API is not overloaded (confirmed via curl).
